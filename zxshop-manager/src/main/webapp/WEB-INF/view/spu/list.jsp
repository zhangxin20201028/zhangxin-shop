		...
		<form action="" id="queryForm">
			<fieldset>
				<label for="key">关键字</label><input id="key" name="key"
					value="${spuvo.key}"> <label for="brand">品牌</label><select
					id="brand" name="brandId">
					<option value="0">--请选择--</option>
					<c:forEach items="${brandList}" var="brand">
						<option value="${brand.id}">${brand.name}</option>
					</c:forEach>
				</select> <label for="sortColumn">排序字段</label> <select id="sortColumn"
					name="orderColumn">
					<option value="">--请选择--</option>
					<option value="goods_name"
						${spuvo.orderColumn=='goodsName'?"selected":''}>名称</option>
					<option value="caption"
						${spuvo.orderColumn=='caption'?"selected":''}>标题</option>
					<option value="category_id"
						${spuvo.orderColumn=='category_id'?"selected":''}>分类</option>
					<option value="brand_id"
						${spuvo.orderColumn=='brand_id'?"selected":''}>品牌</option>
				</select> <label for="sortType">排序方式</label> <input type="radio"
					name="orderType" value="ASC" ${spuvo.orderType=='ASC'?"checked":''}>升序
				&nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="orderType"
					value="DESC" ${spuvo.orderType=='DESC'?"checked":''}>降序
			</fieldset>
			<fieldset>
